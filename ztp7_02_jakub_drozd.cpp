...
//		});
//	
//	PointContainer p2(2, 150, 5, 10);
//	PointContainer a2;
//
//	copy_if(p2.begin(), p2.end(), back_inserter(a2), isNotDominated(p2));
//
//	copy(a2.begin(), a2.end(), back_inserter(a));
//
//	PointContainer nonDominatedA;
//	copy_if(a.begin(), a.end(), back_inserter(nonDominatedA), isNotDominated(a));
//
//	cout << "\nGESTOSCI POZIOM 2:\n" << endl;
//	for_each(nonDominatedA.begin(), nonDominatedA.end(), [&](const Point& pointA) {
//		vector<double> distances;
//		transform(nonDominatedA.begin(), nonDominatedA.end(), back_inserter(distances),
//			[&](const Point& pointB) {
//				return (pointA != pointB) ? define_distance(pointA, pointB) : numeric_limits<double>::max();
//			});
//
